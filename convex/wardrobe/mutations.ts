...
export const removeWardrobeItem = mutation({
  ...
  handler: async (
    ...
    // Delete stored images, ignoring errors if storage object no longer exists
    const tryDelete = async (storageId: Id<'_storage'>) => {
      try {
        await ctx.storage.delete(storageId);
      } catch {
        // Storage object already gone — safe to ignore
      }
    };
    await tryDelete(item.imageStorageId);
    await tryDelete(item.originalImageStorageId);
