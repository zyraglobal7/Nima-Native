...
export const getPendingComments = internalQuery({
  ...
  handler: async (
    ...
    // Hydrate with item names for the AI prompt (catalog + wardrobe)
    return Promise.all(
      recs.map(async (rec) => {
        const items = await Promise.all(
          rec.itemIds.map(async (id: Id<'items'>) => {
        ...
        const wardrobeItems = rec.wardrobeItemIds
          ? await Promise.all(
              rec.wardrobeItemIds.map(async (id: Id<'wardrobeItems'>) => {
                const item = await ctx.db.get(id);
                return item
                  ? { _id: item._id, name: item.description, category: item.category }
                  : null;
              })
            )
          : [];
