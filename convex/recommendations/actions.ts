...
export const generateComments = internalAction({
  ...
  handler: async (
    ...
    for (const rec of recs) {
      try {
        const catalogNames = rec.items.map((i) => i.name);
        const wardrobeNames = (rec.wardrobeItems ?? []).map((i) => i.name);
        const itemList = [...catalogNames, ...wardrobeNames].join(', ');
        const sourceNote = rec.isWardrobeMix && wardrobeNames.length > 0
          ? `These are items from the user's own wardrobe — treat them as "your [item]" not store-bought.`
          : '';
