...
/**
 * Save a quick try-on result to a "Quick Try Ons" lookbook
 */
export const saveQuickTryOnToLookbook = mutation({
  ...
  handler: async (
    ...
    // Find or create "Quick Try Ons" lookbook
    let lookbook = await ctx.db
      .query('lookbooks')
      .withIndex('by_user', (q) => q.eq('userId', user._id))
      .filter((q) => q.eq(q.field('name'), 'Quick Try Ons'))
      .first();
    ...
    if (!lookbook) {
      ...
      const lookbookId = await ctx.db.insert('lookbooks', {
        userId: user._id,
        name: 'Quick Try Ons',
        description: 'My quick try-on results',
        isPublic: false,
    ...
    return { success: true, lookbookId: lookbook._id, message: 'Saved to Quick Try Ons' };
  },
});
